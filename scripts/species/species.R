...
#### joining taxa with bbs taxa
taxa_aou <- taxa %>%
  left_join(
    bbs_data$species %>%
      rename(en_common_name = english)) %>%
  select(order, genus, species, binomial, en_common_name, alpha, aou, 
    intersect_area_pcnt, proposal_focal)
write_rds(taxa_aou, "data/species/prop_prelim.rds")
###
