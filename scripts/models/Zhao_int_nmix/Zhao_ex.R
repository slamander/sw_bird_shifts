...
ebird_sum <- ebird %>%
  summarize(
    ...
    mean_rep_loc = mean(table(Location)),
    med_rep_loc = median(table(Location)),
    max_rep_loc = max(table(Location)),

    mean_rep_fish_id = mean(table(fish_id)),
    med_rep_fish_id = median(table(fish_id)),
    max_rep_fish_id = max(table(fish_id))
  ); ebird_sum
